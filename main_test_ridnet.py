...
def main():
    ...
    args.name = 'ridnet/ridnet_' + args.folder_lq.split('/')[-1]
    ...
    L_path = args.folder_lq                       
    H_path = args.folder_gt

    E_path = os.path.join(args.results, args.name)   # E_path, for Estimated images
    util.mkdir(E_path)

    # if H_path == L_path:
    #     args.need_degradation = True
    logger_name = args.folder_lq.split('/')[-1]
    ...
    for idx, img in enumerate(L_paths):
        ...
        if need_H:
            ...
            lpips_ = loss_fn_alex(im2tensor(img_E).cuda(), im2tensor(img_H).cuda()).item()
            test_results['psnr'].append(psnr)
            test_results['ssim'].append(ssim)
            test_results['lpips'].append(lpips_)

            # logger.info('{:s} - PSNR: {:.2f} dB; SSIM: {:.4f}; LPIPS: {:.4f}'.format(img_name+ext, psnr, ssim, lpips_))
    ...
    if need_H:
        ...
        logger.info('Average PSNR/SSIM(RGB)/LPIPS - {} - PSNR/SSIM/LPIPS: {:.2f}/{:.4f}/{:.4f}'.format(args.model_name, ave_psnr, ave_ssim, ave_lpips))
